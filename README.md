...
-   fs - fileSystem module to interacting with files on a machine
    ...
    const file = fs.readFileSync('./demo-text.txt', { encoding: 'utf-8' }).toString();
...
Example of `HBS` Template engine
...
It is a **software** that allows two application to communicate with each other over the internet and through various devices. Every time you access an app like Facebook or check the weather on your smartphone, an API is used.
...
restFull api use curd

We perform CURD operation to create RESTful API
...
-   **UPDATE**
    -   PUT : which replaces the original version of the resource
    -   PATCH : PATCH method supplies a set of instructions to modify the resource.
-   **DELETE**
    -   DELETE
...
**Wrong way** to create RESTful API
...
**Right way** to create RESTful API
